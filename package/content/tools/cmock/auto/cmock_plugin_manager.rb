    ...
    plugins_to_load.each do |plugin|
      ...
      begin
        unless (Object.const_defined? object_name)
          plugin_file = "#{File.expand_path(File.dirname(__FILE__))}/cmock_generator_plugin_#{plugin_name.downcase}.rb"
          raise "plugin '#{plugin_name}' not found." unless File.exists?(plugin_file)
          require "#{plugin_file}"
        end
