...
class CMockGenerator
  ...
  attr_accessor :config, :file_writer, :module_name, :clean_mock_name, :mock_name, :utils, :plugins, :ordered, :relative_path
  ...
  def create_mock_header_file(parsed_stuff)
    @file_writer.create_file(@mock_name + ".h") do |file, filename|
      ...
      create_mock_header_service_call_declarations(file, parsed_stuff[:externd_variables])
  ...
  def create_mock_source_file(parsed_stuff)
    @file_writer.create_file(@mock_name + ".c") do |file, filename|
      ...
      create_extern_declarations(file, parsed_stuff[:externd_variables])
      create_mock_verify_function(file, parsed_stuff[:functions])
      create_mock_init_function(file)
      create_mock_destroy_function(file, parsed_stuff[:functions], parsed_stuff[:externd_variables])
      parsed_stuff[:functions].each do |function|
  ...
  def create_mock_header_service_call_declarations(file, externd_variables)
    file << "void #{@clean_mock_name}_Init(void);\n"
    file << "void #{@clean_mock_name}_Destroy(void);\n"
    file << "void #{@clean_mock_name}_Verify(void);\n\n"
    externd_variables.each { |var| file << "extern " + var[:declaration] + "\n" }
  end
  ...
  def create_extern_declarations(file, externd_variables)
    file << "#if !defined(__XC8)\n"
    ...
    externd_variables.each { |var| file << var[:declaration] + "\n" }
    file << "\n"
  end
  ...
  def create_mock_destroy_function(file, functions, externd_variables)
    file << "void #{@clean_mock_name}_Destroy(void)\n{\n"
    ...
    externd_variables.each { |var|
      name = var[:name]
      file << "  #{name} = 0;\n" if var[:simple_type] && !var[:is_ptr] && !var[:is_array]
      file << "  #{name} = NULL;\n" if var[:is_ptr]
      file << "  memset(&#{name}, 0, sizeof(#{name}));\n" if var[:is_array] || !(var[:simple_type] || var[:is_ptr])
    }
      file << "}\n\n"
  end
