...
class CMock
  ...
  def initialize(options=nil)
    ...
    @cm_stubber    = CMockStubGenerator.new(cm_config)
  end
  ...
  def generate_mock(src)
    ...
    parsed_stuff = @cm_parser.parse(name, File.read(src))
    @cm_generator.create_mock(name, parsed_stuff)
    @cm_stubber.create_stub(File.basename(src), File.read(src), parsed_stuff)
  end
end
